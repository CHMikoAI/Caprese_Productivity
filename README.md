...
- **Partner** — the "find a life partner" track, run week by week. Each week holds two or three small, independent goals; one week is in focus at a time (arrows page through it, *The weeks so far* jumps around). A goal can only be closed **together with its debrief**: how hard it felt (1–5) and a line on what happened — the database enforces the pairing, so nothing is ever ticked off unreflected. Once the week is done it opens a wrap-up: *what's the one thing you take into next week?* Starting an empty week from the last one takes one tap (*Repeat last week's goals*). Four stats carry the progress: this week, goals done all time, a streak of weeks in a row, and **Feels like** — the average difficulty of the last four weeks against the four before, so you can see the same kind of goal getting easier. Picks: +1 per closed goal, +2 more once every goal of a week is closed (idempotent — reopening and closing again never pays twice).
- **Pantry (gamification)** — doing things earns *picks*: journal entry +1, completed task +2, achieved goal +3, closed Partner goal +1 (+2 for a fully closed week), +1 for every 3 completed to-dos, and +2 for every 10 consecutive journal days. Each pick draws one card (flip it to reveal): tomato 50 %, basil 20 %, olive oil 20 %, mozzarella 10 % — rolled per card, server-side. Collect 5 tomatoes + 2 basil + 2 olive oil + 1 mozzarella to craft a caprese salad, then redeem a salad for a private treat (with a note of what it was). Awards are idempotent (a ledger with unique source keys), drawing and crafting run atomically in Postgres functions.
- **Keyboard shortcuts** — app-style single keys (never while typing, never in dialogs): `1–5` switch pages, `?` shows the full overview, `Esc` closes dialogs. Calendar mirrors Google Calendar: `N` new, `T` today, `D`/`W`/`M` views, `←`/`→` navigate, `S` sidebar. Planner: `N` new, `Q` quick add, `/` search. Journal: `N` write. Partner: `N` add a goal. Pantry: `D` draw a card.
...
2. **Database** — the schema (`categories`, `entries`, `journal_entries`, `partner_goals`, `thoughts`, pantry tables) is already provisioned in the Supabase project "Caprese App". To recreate it elsewhere, run [`supabase/schema.sql`](supabase/schema.sql) in the SQL editor.
